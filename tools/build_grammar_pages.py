...
function speakJa(text) {
  try {
    ...
    // 「食べる たべる」「かえる 帰る」這種漢字與假名並列的儲存格，整串唸出來會
    // 把同一個字唸兩次。有純假名的那一段就只唸它——假名對 TTS 沒有讀音歧義。
    if (/\\s/.test(t)) {
      const kana = t.split(/\\s+/).find(s => /^[ぁ-んァ-ヶーヽヾ゛゜]+$/.test(s));
      if (kana) t = kana;
    }
    if (!t) return;
    _gen += 1;
...
document.addEventListener('DOMContentLoaded', () => {
  // 欄名白名單——只有「這一欄裝的是日文」才掛朗讀鍵。漏了活用形那幾欄的話，
  // 第6章的ます形與四個時態、第7章的て形全部沒有聲音，那正是最需要聽的地方。
  const SPEAK_COLS = ['日文', '例句', '例', '例子', '例字', '句型', '假名', '讀法',
                      '原形', '辭書形', '動詞', '禮貌體', 'ます形', 'て形', '＋する',
                      '現在肯定', '現在否定', '過去肯定', '過去否定'];
...
<main class="grammar-index">
  <header class="chapter-head">
    ...
    <p class="lead note">左邊的兩位數是<strong>教學順序</strong>；標題裡的 G 編號是<a href="../notes/#/grammar.md">文法地圖</a>上的編號，兩者刻意不一致——順序依相依性調整過，例如 G9〜てください 依賴 G6 て形，所以排在它後面先教，G7〜ている 反而排到後面。</p>
  </header>
  <ul class="entries">
{chr(10).join(lis)}
  </ul>{pending_p}
